...
namespace SeguriGasesERP.Models
{
    ...
    public class Cliente
    {
        ...
        {
            //Primero buscamos si existe una cuenta de crédito a nombre del cliente
            var CuentaCredito = db.CuentasCredito.Where(c => c.IdCliente == ID);

            if (CuentaCredito.Count() == 0 || CuentaCredito == null)
                return false;
            ...
            //Verificamos si no se ha llegado alímite de crédito en días
            //Lo desactivamos por el momento

            /*
                if (CuentaCredito.First().VentasVencidas())
                    return false;
             * 
             * */
            ...
            //Verificamos si no excede su límite de credito en dinero
            if ((CuentaCredito.First().getDeuda() + proximaVenta) > CuentaCredito.First().MontoLimite)
                return false;
